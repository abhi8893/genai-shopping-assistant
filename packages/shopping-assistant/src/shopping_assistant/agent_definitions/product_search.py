...
class ProductSearchAgent:
    ...
    def run(self, state: State) -> State:
        if state.prev_recommended_products is not None:
            prev_recommended_products_prompt_str = get_product_list_prompt_str(
                state.prev_recommended_products
            )
        else:
            prev_recommended_products_prompt_str = (
                "No products have been recommended to the user so far."
            )
        ...
        product_retrieval_response = retrieve_products(
            ...
            categories=[prod_retrieval_agent_response.product_details.category]
            if prod_retrieval_agent_response.product_details.category
            else None,
            subcategories=[prod_retrieval_agent_response.product_details.subcategory]
            if prod_retrieval_agent_response.product_details.subcategory
            else None,
        ...
        products_retrieved = [
            ProductVectorDBRecord(**p.properties)
            for p in product_retrieval_response.objects
        ]

        asst_response = self._create_product_retrieval_response(
