...
#     def update_id_to_slug(self, cart_items: list[shopping_assistant.types.CartItem]):
...
class Cart:
    ...
    def _get_counter_from_cart_items(self, cart_items: list[shopping_assistant.types.CartItem]) -> Counter:

        counter = Counter()
        for cart_item in cart_items:
            cart_item = shopping_assistant.types.CartItem.model_validate(cart_item)
    ...
    def _get_cart_items_from_counter(self, counter: Counter, as_dict=True) -> list[CartItemData] | list[dict]:
        ...
        for product_slug, quantity in counter.items():
            ...
            cart_item = shopping_assistant.types.CartItem(product_id=product_id, quantity=quantity)
            if as_dict:
