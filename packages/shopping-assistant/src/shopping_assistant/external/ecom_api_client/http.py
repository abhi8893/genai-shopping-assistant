...
class HttpClient:
    def __init__(
        self, base_url: str, credentials: Credentials | None = None, prefix=""
    ):
        self.base_url = base_url + prefix
        self.session = requests.Session()
        self.credentials = (
            credentials if credentials is not None else Credentials(user_id=None)
        )

    def request(
        self, method, path, raise_error=False, **kwargs
        ...
        if isinstance(response_json, dict):  # noqa: SIM102
            ...
            if response_json.get("error_code", "") == "NOT_FOUND":
                raise ApiError(
                    ExceptionResponse(
                        success=False, detail="Not Found", error_code="NOT_FOUND"
                    )
                )
