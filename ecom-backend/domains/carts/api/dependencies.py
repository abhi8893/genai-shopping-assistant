...
def get_sqlalchemy_product_repo(db: Session = Depends(get_db)) -> SQLAlchemyProductRepository:
    return SQLAlchemyProductRepository(db)


def get_product_service(repo: SQLAlchemyProductRepository = Depends(get_sqlalchemy_product_repo)) -> ProductService:
    return ProductService(repo)
    
def get_cart_service(repo: SQLAlchemyCartRepository = Depends(get_sqlalchemy_cart_repo), product_service: ProductService = Depends(get_product_service)) -> CartService:
